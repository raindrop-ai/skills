...
- **Experiment outcomes.** Start with `get_experiment` to find a saved comparison. For a new experiment, ask what better means: signals, users or events, and direction. Save that rule in `module_config.signals.success`, separate from cohort filters. Report the returned verdict and confidence; do not invent a second winner from signal deltas. See the [experiment tool reference](references/mcp-tools.md#experiments).
